...
2. Send an API request to Raspberry to generate OAuth creds
3. Create an endpoint to accept key change notifications

*/

// TODO: Actually redirect the client on the passthrough
...
type NewOAuthNofication struct {
	AuthCode         string
...
// OAuthHandlers are the HTTP Handlers that manage the Raspberry OAuth flow
...
func (o *OAuthHandlers) generateOAuthOutputFromOsinResponse(osinResponse *osin.Response) ([]byte, bool) {
	...
	if rediErr != nil {
		...
		osinResponse.Output["reirect_to"] = redirect
	}
...
func (o *OAuthHandlers) notifyClientOfNewOAuth(notification NewOAuthNofication) bool {
...
func (o *OAuthHandlers) HandleAccessRequest(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" || r.Method == "POST" {
		...
		responseMessage, _ = o.generateOAuthOutputFromOsinResponse(resp)
		if resp.IsError {
			// Something went wrong, wrote out the error details and kill the response
			w.WriteHeader(resp.ErrorStatusCode)
			fmt.Fprintf(w, string(responseMessage))
			return
		}
		...
		newNotification := NewOAuthNofication{
			AuthCode:         code,
...
func (r RedisOsinStorageInterface) LoadAuthorize(code string) (*osin.AuthorizeData, error) {
	...
	if storeErr != nil {
		log.Error("Failure retrieving auth code key")
		log.Error(storeErr)
		return nil, storeErr
	}
...
func (r RedisOsinStorageInterface) SaveAccess(accessData *osin.AccessData) error {
	...
	if accessData.RefreshToken != "" {
		authDataJSON, marshalErr := json.Marshal(&accessData)
		if marshalErr != nil {
			log.Error("Counln't marshal AccessData")
			log.Error(marshalErr)
			return marshalErr
		}
...
func (r RedisOsinStorageInterface) LoadAccess(token string) (*osin.AccessData, error) {
	...
	if storeErr != nil {
		log.Error("Failure retrieving access token by key")
		log.Error(storeErr)
		return nil, storeErr
	}
